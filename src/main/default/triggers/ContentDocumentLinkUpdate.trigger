trigger ContentDocumentLinkUpdate on ContentDocumentLink(after update, after delete, after insert ){
	...
	For (ContentDocument cd : [select id, title, CreatedDate, OwnerId
	                           from ContentDocument
	                           where id in :listCD]){
		cs.put(cd.id, cd);
	}

	For (Solar_Loans__c solarLoan : [select id, Member_Number__c
	                                 from Solar_Loans__c
	                                 where id in :parent]){
		sl.put(solarLoan.id, solarLoan);
	}

	For (xPressRefi__c xrefi : [select id, Member_Number__c
	                            from xPressRefi__c
	                            where id in :parent]){
		xl.put(xrefi.id, xrefi);
	}

	For (ContentVersion contentVersion : [select id, NewFile__c, ContentDocumentId
	                                      from ContentVersion
	                                      where ContentDocumentId in :listCD]){
	...
	if (Trigger.isInsert && Trigger.isAfter){
		for (ContentDocumentLink c : Trigger.New){
            ...
            //-------------------------End - LT11477-81: Adding Document to xPressRefi Document Object---------------------------------//
		}
	}
	...
	for (Member_Comment__c mem : [select Id, Name, Attachment_Number__c, (SELECT Id
	...
	for (Solar_Loans__c sl : [select Id, count__c, (SELECT Id
	                                                FROM SolarLoan_Documents__r)
	                          from Solar_Loans__c
	                          where Id IN :parent]){
