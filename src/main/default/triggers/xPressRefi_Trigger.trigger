trigger xPressRefi_Trigger on xPressRefi__c(after insert, after update ){
	...
	if (Trigger.isAfter && Trigger.isInsert){
		for (Integer i = 0; i < trigger.new.size(); i++){
			if(trigger.new[i].Impermissible__c == true && trigger.new[i].is_CSV_Generated__c == false){
				xPressRefiIdsforMortgageCadenceCSV.add(trigger.new[i].Id);
			}
			else if (trigger.new[i].Is_Product_Change__c == false && trigger.new[i].Impermissible__c != true){
